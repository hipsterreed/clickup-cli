...
program
    ...
    const cmd = (s) => c.green(s.padEnd(52));
...
program
    ...
    .action((opts) => {
    const { clear } = render(React.createElement(SetupApp, {
...
program
    ...
    rl.question(chalk.yellow('Remove stored credentials? (y/N) '), (answer) => {
        rl.close();
...
program
    ...
    if (!opts.json)
        process.stdout.write(chalk.gray('Fetching spaces...\r'));
    try {
...
program
    ...
    const { teamId } = getConfig();
    if (!opts.json)
        process.stdout.write(chalk.gray('Fetching lists...\r'));
    try {
        const results = [];
        if (opts.folder) {
        ...
        if (opts.json) {
            ...
            console.log(chalk.gray('  ' + 'ID'.padEnd(22) + 'NAME'.padEnd(32) + 'SPACE / FOLDER'));
            console.log(chalk.gray('  ' + '─'.repeat(80)));
            for (const l of results) {
                const location = l.folder ? `${l.space.name} / ${l.folder.name}` : l.space.name;
                console.log(`  ${chalk.cyan(l.id.padEnd(22))}${l.name.padEnd(32)}${chalk.gray(location)}`);
            }
            console.log();
        }
    }
...
async function runFlagMode(filters, showDesc, json) {
    if (!json)
    ...
    try {
        ...
        if (json) {
            ...
            const { unmount } = render(React.createElement(QuickTaskList, { tasks: taskList, showDesc }));
...
tasks
    ...
    try {
        ...
        if (opts.json) {
            ...
            const { unmount } = render(React.createElement(QuickTaskDetail, { task: t, showComments: opts.comments }));
    ...
    if (opts.name) {
        try {
            ...
            if (opts.due) {
                ...
                if (isNaN(dueDateMs)) {
                    if (opts.json)
                        outputJsonError('Invalid due date. Use YYYY-MM-DD format.');
                    else {
